...
def fancy_traceback(exc: Exception) -> str:
    """Может не соответствовать ограничению содержимого сообщения"""
...
class Events(commands.Cog):
    ...
    async def on_command_error(self, ctx: commands.Context, error: commands.CommandError):
      embed = disnake.Embed(
        title=f"Команда `{ctx.command}` вызвала ошибку: `{error}`",
    ...
    async def on_slash_command_error(self, inter: disnake.AppCmdInter, error: commands.CommandError):
      embed = disnake.Embed(
        title=f"Слэш команда `{inter.data.name}` потерпела неудачу из-за `{error}`",
        description=fancy_traceback(error),
        color=config.error,
      )
    ...
    async def on_user_command_error(self, inter: disnake.AppCmdInter, error: commands.CommandError):
      embed = disnake.Embed(
        title=f"Пользовательская команда `{inter.data.name}` потерпела неудачу из-за `{error}`",
        description=fancy_traceback(error),
        color=config.error,
      )
      if inter.response._responded:
    ...
    async def on_message_command_error(self, inter: disnake.AppCmdInter, error: commands.CommandError):
      embed = disnake.Embed(
        title=f"Команда сообщения`{inter.data.name}` потерпела неудачу из-за `{error}`",
        description=fancy_traceback(error),
        color=config.error,
      )
      if inter.response._responded:
